...
public class OutStreamOptionsTest {
  ...
  @Test
  public void testCreateFilePOptionsSetReplicationMin() {
    // 1. Configuration as Input
    AlluxioConfiguration conf = new InstancedConfiguration(new AlluxioProperties());
    ...
    // 3. Invoke the method under test
    CreateFilePOptions options = FileSystemOptions.createFileDefaults(conf);
    CreateFilePOptions.Builder builder = options.toBuilder().setReplicationMin(expectedReplicationMin);
    CreateFilePOptions newOptions = builder.build();

    // 4. Assertions and Verification
    assertEquals(expectedReplicationMin, newOptions.getReplicationMin());
  }
}