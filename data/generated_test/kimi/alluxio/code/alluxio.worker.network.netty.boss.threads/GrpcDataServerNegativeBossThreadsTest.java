...
public class GrpcDataServerNegativeBossThreadsTest {
    ...
    public void negativeBossThreadsThrowsIllegalArgument() {
        // 1. Create a new Configuration and set PropertyKey.WORKER_NETWORK_NETTY_BOSS_THREADS to -3
        //    (done via mock below, so no explicit conf.set(...))

        // 2. Mock ServerConfiguration.getInt(PropertyKey.WORKER_NETWORK_NETTY_BOSS_THREADS) to return -3
        PowerMockito.when(ServerConfiguration.getInt(PropertyKey.WORKER_NETWORK_NETTY_BOSS_THREADS))
                .thenReturn(-3);

        // 3. Attempt to instantiate GrpcDataServer
