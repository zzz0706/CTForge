...
public class LocalFirstAvoidEvictionPolicyTest {
  ...
  @Test
  public void equalsConsidersReservedBytes() {
    // 1. Load default configuration
    AlluxioConfiguration conf = new InstancedConfiguration(
        ConfigurationUtils.defaults());

    // 2. Prepare test conditions: create policies with different reserved bytes
    ((InstancedConfiguration) conf).set(PropertyKey.USER_BLOCK_AVOID_EVICTION_POLICY_RESERVED_BYTES, "256MB");
    LocalFirstAvoidEvictionPolicy policy256 = new LocalFirstAvoidEvictionPolicy(conf);

    ((InstancedConfiguration) conf).set(PropertyKey.USER_BLOCK_AVOID_EVICTION_POLICY_RESERVED_BYTES, "512MB");
    LocalFirstAvoidEvictionPolicy policy512 = new LocalFirstAvoidEvictionPolicy(conf);

    // 3. Verify equals and hashCode behavior
    new EqualsTester()
        .addEqualityGroup(policy256)
        .addEqualityGroup(policy512)
        .testEquals();
  }
}