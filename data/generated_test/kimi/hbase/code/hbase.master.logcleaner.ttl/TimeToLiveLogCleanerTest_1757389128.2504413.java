...
public class TimeToLiveLogCleanerTest {
  ...
  public void negativeLifeSkipsDeletion() {
    // 1. Use HBase 2.2.2 API to obtain configuration value
    Configuration conf = new Configuration();
    // no override, rely on default TTL from TimeToLiveLogCleaner
  ...
  @Test
  public void nonExpiredFileIsNotDeletable() {
    // 1. Use HBase 2.2.2 API to obtain configuration value
    Configuration conf = new Configuration();
    conf.setLong(TimeToLiveLogCleaner.TTL_CONF_KEY, 60000L); // 60 s TTL
    ...
    when(mockPath.getName()).thenReturn("logfile.1234567890");
    when(mockFileStatus.getPath()).thenReturn(mockPath);
    long recentTime = System.currentTimeMillis() - 30000; // 30 seconds ago
    when(mockFileStatus.getModificationTime()).thenReturn(recentTime);

    // 3. Test code
    boolean deletable = cleaner.isFileDeletable(mockFileStatus);

    // 4. Post-test verification
    assertFalse("Non-expired file should not be deletable", deletable);
  }
}