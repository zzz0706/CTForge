...
public class TestTimeToLiveLogCleanerDefaultTtl {
  ...
  public void defaultTtlUsedWhenNoOverride() {
    ...
    Configuration conf = new Configuration(false); // empty, no overrides

    // 2. Prepare the test conditions.
    TimeToLiveLogCleaner cleaner = new TimeToLiveLogCleaner();
    cleaner.setConf(conf); // loads default TTL via TTL_CONF_KEY

    final long now = 1672531200000L; // fixed time for reproducibility
    final long fileTime = now - 600000L - 1; // exactly DEFAULT_TTL + 1 ms in the past
