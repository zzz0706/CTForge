...
public class TestCompactionConfiguration {
  ...
  @Test
  public void testMinCompactSizeZeroAllowsAllFilesRatioBased() throws Exception {
    // 1. Configuration as input
    Configuration conf = HBaseConfiguration.create();
    conf.setLong(CompactionConfiguration.HBASE_HSTORE_COMPACTION_MIN_SIZE_KEY, 0L);
    ...
    // Provide file sizes to satisfy compaction policy logic
    when(reader.length()).thenReturn(1024L);

    ArrayList<HStoreFile> candidates = new ArrayList<>(Arrays.asList(sf1, sf2, sf3));

    // 3. Test code
    ArrayList<HStoreFile> result = policy.applyCompactionPolicy(candidates, false, false);
