...
public class TestConnectionManagerIdleScanThreshold {
  ...
  @Test
  public void closeIdleHonorsIdleScanThreshold() throws Exception {
    // 1. You need to use the hdfs 2.8.5 API correctly to obtain configuration values, instead of hardcoding the configuration values.
    Configuration conf = new Configuration();
    conf.setInt(CommonConfigurationKeysPublic.IPC_CLIENT_IDLETHRESHOLD_KEY, 2);
    ...
    Field thresholdField = connectionManager.getClass().getDeclaredField("idleScanThreshold");
    thresholdField.setAccessible(true);
    int idleScanThreshold = (int) thresholdField.get(connectionManager);

    assertEquals(2, idleScanThreshold);
    ...
    // Trigger closeIdle with scanAll=false to exercise the threshold check
    closeIdleMethod.invoke(connectionManager, false);

    // 4. Code after testing.
    // No exception and coverage of the threshold check inside closeIdle
  }
}