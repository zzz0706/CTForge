...
public class NativeIOCacheTimeoutTest {
    ...
    public void testFstatUsesCacheTimeoutForOwnerLookup() throws Exception {
        ...
        // Use reflection to set the cacheTimeout field directly
        java.lang.reflect.Field cacheTimeoutField = NativeIO.class.getDeclaredField("cacheTimeout");
        cacheTimeoutField.setAccessible(true);
        cacheTimeoutField.setLong(null, 1000L); // 1 second
        ...
        try (FileOutputStream fos = new FileOutputStream(tmp)) {
            ...
            when(System.currentTimeMillis())
                    .thenReturn(baseTime)
                    .thenReturn(baseTime + 1001L); // Advance by 1001ms

            // 3. Second call should trigger fresh lookups
            NativeIO.POSIX.Stat stat2 = NativeIO.POSIX.getFstat(fd);

            // 4. Verify that fresh lookups occurred (owner/group names should still match)
            assertNotNull(stat2);
