...
public class ChecksumFileSystemConfigCoverageTest {
    ...
    @Test
    public void testGetSumBufferSizeWithSmallBuffer() throws Exception {
        // 1. Configuration as input
        conf.setInt(LocalFileSystemConfigKeys.LOCAL_FS_STREAM_BUFFER_SIZE_KEY, 512);
        LocalFileSystem customLfs = FileSystem.getLocal(conf);

        // 2. Prepare the test conditions
        Path smallBufFile = new Path(System.getProperty("java.io.tmpdir"), "testfile_smallbuf");
        customLfs.delete(smallBufFile, true);
        FSDataOutputStream out = customLfs.create(smallBufFile);
        byte[] data = new byte[1024];
        out.write(data);
        out.close();

        // 3. Test code - Verify getSumBufferSize is used correctly
        FSDataInputStream in = customLfs.open(smallBufFile);
        assertNotNull(in);
        ...
        assertTrue(customLfs.exists(customLfs.getChecksumFile(smallBufFile)));
