...
public class StandbyCheckpointerTest {
    ...
    public void testCheckpointCheckPeriodInStandbyCheckpointer() throws Exception {
        ...
        when(conf.getLong(
            ...
            eq(DFSConfigKeys.DFS_NAMENODE_CHECKPOINT_CHECK_PERIOD_DEFAULT))).thenReturn(checkPeriodValue);
            
        when(conf.getLong(
            eq(DFSConfigKeys.DFS_NAMENODE_CHECKPOINT_PERIOD_KEY),
            eq(DFSConfigKeys.DFS_NAMENODE_CHECKPOINT_PERIOD_DEFAULT))).thenReturn(checkpointPeriodValue);
            
        when(conf.getLong(
            eq(DFSConfigKeys.DFS_NAMENODE_CHECKPOINT_TXNS_KEY),
            eq(DFSConfigKeys.DFS_NAMENODE_CHECKPOINT_TXNS_DEFAULT))).thenReturn(DFSConfigKeys.DFS_NAMENODE_CHECKPOINT_TXNS_DEFAULT);
            
        when(conf.getInt(
            eq(DFSConfigKeys.DFS_NAMENODE_CHECKPOINT_MAX_RETRIES_KEY),
            eq(DFSConfigKeys.DFS_NAMENODE_CHECKPOINT_MAX_RETRIES_DEFAULT))).thenReturn(DFSConfigKeys.DFS_NAMENODE_CHECKPOINT_MAX_RETRIES_DEFAULT);
