...
public class TestClientRPCConfiguration {
    ...
    public void testClient_getTimeout_withPingDisabled() {
        ...
        assertEquals("RPC timeout does not match the expected ping interval!", expectedTimeout, timeout);
    }
    ...
    // Test code covering 'RPC.waitForProtocolProxy(Class<T>, long, InetSocketAddress, Configuration, long)'
    @Test
    public void testRPC_waitForProtocolProxy() throws IOException {
        ...
        // Call API
        ProtocolProxy<TestProtocolImpl> proxy = RPC.waitForProtocolProxy(TestProtocolImpl.class, 1L, addr, conf, connTimeout);

        // Validate results
        assertNotNull("Protocol proxy should not be null!", proxy);
    }

    // Test code covering 'RPC.getProtocolProxy(Class<T>, long, InetSocketAddress, UserGroupInformation, Configuration, SocketFactory)'
    @Test
    public void testRPC_getProtocolProxy() throws IOException {
        ...
        // Call API
        ProtocolProxy<TestProtocolImpl> proxy = RPC.getProtocolProxy(TestProtocolImpl.class, 1L, addr, ticket, conf, factory);

        // Validate results
        assertNotNull("Protocol proxy should not be null!", proxy);
    }

    // Simple interface for testing protocol proxy instantiation
