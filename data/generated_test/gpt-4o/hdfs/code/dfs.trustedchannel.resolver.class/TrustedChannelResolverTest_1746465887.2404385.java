...
public class TrustedChannelResolverTest {
    ...
    @Test
    // Ensure `newInstance` in ReflectionUtils throws a RuntimeException when attempting to instantiate a class that does not exist.
    // 1. You need to use the HDFS 2.8.5 ReflectionUtils API correctly.
    // 2. Prepare the test conditions.
    // 3. Test code.
    // 4. Code after testing.
    public void test_newInstance_withInvalidClass() {
        // 1. Prepare the test condition: Create a Configuration object
        Configuration conf = new Configuration();

        try {
            // 2. Attempt to create an instance of a non-existent class using ReflectionUtils
            Class<?> nonExistentClass = Class.forName("com.example.NonExistentClass");
            ReflectionUtils.newInstance(nonExistentClass, conf);

            // 3. Test code: The test should fail if no exception is thrown
            fail("Expected RuntimeException due to invalid class, but no exception was thrown.");
        } catch (ClassNotFoundException e) {
            // 4. Code after testing: Handle ClassNotFoundException (expected)
            assertTrue("Exception message should indicate the class was not found.",
                    e.getMessage().contains("com.example.NonExistentClass"));
        } catch (RuntimeException e) {
            // 4. Code after testing: Handle RuntimeException from ReflectionUtils
            assertTrue("RuntimeException should be thrown for class instantiation failure.",
                    e.getMessage().contains("com.example.NonExistentClass"));
        }
    }
}