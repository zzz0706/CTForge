...
public class TestWarmUpEdekCacheInvalidConfiguration {
    ...
    public void testWarmUpEdekCacheWithInvalidConfiguration() throws IOException {
        ...
        conf.setInt(DFSConfigKeys.DFS_NAMENODE_EDEKCACHELOADER_INTERVAL_MS_KEY, -5); // Invalid interval configuration
        ...
        try {
            // Step 2: Invoke warmUpEdekCache with invalid configuration values
            FSDirEncryptionZoneOp.warmUpEdekCache(executor, fsDirectory, edekCacheLoaderDelay, edekCacheLoaderInterval);
        } catch (Exception e) {
            // Step 3: Log the exception if the process fails with invalid configuration
