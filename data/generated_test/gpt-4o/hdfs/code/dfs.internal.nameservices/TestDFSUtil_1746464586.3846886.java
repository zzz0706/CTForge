...
public class TestDFSUtil {
    ...
    @Test
    // test_getNNServiceRpcAddressesForCluster_withValidInternalNameservices
    ...
    @Test
    // test_getNNLifelineRpcAddressesForCluster_withValidInternalNameservices
    // 1. You need to use the hdfs 2.8.5 API correctly to obtain configuration values, instead of hardcoding the configuration values.
    // 2. Prepare the test conditions.
    // 3. Test code.
    // 4. Code after testing.
    ...
    @Test
    // test_getInternalNameServices_Configuration
    // 1. You need to use the hdfs 2.8.5 API correctly to obtain configuration values, instead of hardcoding the configuration values.
    // 2. Prepare the test conditions.
    // 3. Test code.
    // 4. Code after testing.
    public void test_getInternalNameServices_Configuration() {
        // Prepare the test conditions: create a Configuration instance
        Configuration conf = new Configuration();

        // Set up dfs.internal.nameservices configuration
        conf.set("dfs.internal.nameservices", "ns1,ns2");

        // Test code: call getInternalNameServices()
        Collection<String> internalNameServices = DFSUtil.getInternalNameServices(conf);

        // Assertions: ensure the function returns the expected collection
        assertNotNull("Internal nameservices collection should not be null", internalNameServices);
        assertTrue("Internal nameservices should contain 'ns1'", internalNameServices.contains("ns1"));
        assertTrue("Internal nameservices should contain 'ns2'", internalNameServices.contains("ns2"));

        // Print values for debugging
        System.out.println("Internal NameServices: " + internalNameServices);
    }

    @Test
    // test_getInternalNsRpcUris_Configuration
    // 1. You need to use the hdfs 2.8.5 API correctly to obtain configuration values, instead of hardcoding the configuration values.
    // 2. Prepare the test conditions.
    // 3. Test code.
    // 4. Code after testing.
    public void test_getInternalNsRpcUris_Configuration() {
        // Prepare the test conditions: create a Configuration instance
        Configuration conf = new Configuration();

        // Set up valid dfs.internal.nameservices configuration and corresponding RPC addresses
        conf.set("dfs.internal.nameservices", "ns1,ns2");
        conf.set("dfs.namenode.service.rpc-address.ns1", "localhost:8024");
        conf.set("dfs.namenode.service.rpc-address.ns2", "localhost:8025");
        conf.set("dfs.namenode.rpc-address.ns1", "localhost:8020");
        ...
        // Test code: call getInternalNsRpcUris()
        Collection<URI> internalNsRpcUris = DFSUtil.getInternalNsRpcUris(conf);

        // Assertions: ensure the function returns the expected URIs
        assertNotNull("Internal nameservice URIs collection should not be null", internalNsRpcUris);
        assertTrue("Internal nameservice URIs should not be empty", !internalNsRpcUris.isEmpty());

        // Print values for debugging
        System.out.println("Internal Nameservice RPC URIs: " + internalNsRpcUris);
    }
}