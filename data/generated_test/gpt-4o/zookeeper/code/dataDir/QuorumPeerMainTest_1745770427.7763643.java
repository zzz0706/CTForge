...
public class QuorumPeerMainTest {
    ...
    @Test
    // test code for DatadirCleanupManager#start
    // 1. You need to use the zookeeper3.5.6 API correctly to obtain configuration values, instead of hardcoding the configuration values.
    // 2. Prepare the test conditions.
    // 3. Test code.
    // 4. Code after testing.
    public void test_DatadirCleanupManager_start() throws Exception {
        // Step 1: Prepare Mock Cleanup Manager
        File mockDataDir = new File("/tmp/zookeeper/dataDir");
        File mockDataLogDir = new File("/tmp/zookeeper/dataLogDir");
        int snapRetainCount = 3;
        int purgeInterval = 12;

        DatadirCleanupManager cleanupManager = spy(new DatadirCleanupManager(
            mockDataDir,
            mockDataLogDir,
            snapRetainCount,
            purgeInterval
        ...
        // Step 2: Execute Start Logic
        cleanupManager.start();

        // Validate Behavior
        verify(cleanupManager, times(1)).start();
    }
}